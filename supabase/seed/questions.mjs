...
export const QUESTIONS = [
  ...
  // ═══ Insolite ═════════════════════════════════════════════════════════
  // Les questions qui font rire avant de faire chercher. Elles sont
  // volontairement faciles a comprendre et difficiles a prendre au serieux,
  // et ce sont celles dont on reparle a la fin de la soiree.
  ecrite('Insolite', 1, 'De quelle couleur était le cheval blanc d’Henri IV ?',
    ['Blanc', 'Blanche']),
  ecrite('Insolite', 1, 'Combien de mois de l’année comptent 28 jours ?',
    ['12', 'Douze', 'Tous'], 'Relisez bien la question'),
  ecrite('Insolite', 2, 'Un avion s’écrase pile sur la frontière. Où enterre-t-on les survivants ?',
    ['Nulle part', 'On ne les enterre pas', 'Aucun', 'On n’enterre pas les survivants'],
    'Les survivants vont plutôt bien'),
  ecrite('Insolite', 2, 'Quel animal est incapable de reculer ?',
    ['Kangourou', 'Le kangourou', 'Émeu']),
  ecrite('Insolite', 2, 'Quel aliment ne se périme jamais ?',
    ['Miel', 'Le miel']),
  ecrite('Insolite', 2, 'Quel mammifère dort jusqu’à vingt heures par jour ?',
    ['Koala', 'Le koala']),
  ecrite('Insolite', 2, 'Comment appelle-t-on le cri du chameau ?',
    ['Blatèrement', 'Il blatère', 'Blatérer']),
  ecrite('Insolite', 2, 'Quel fruit contient plus de quatre-vingt-dix pour cent d’eau ?',
    ['Pastèque', 'La pastèque', 'Melon d’eau']),
  ecrite('Insolite', 3, 'Combien de nez a une limace ?',
    ['4', 'Quatre'], 'Plus d’un, en tout cas'),
  estimation('Insolite', 2, 'Combien de fois un humain cligne-t-il des yeux par jour ?',
    15000, 'fois'),
  estimation('Insolite', 2, 'Combien de litres de salive produit-on par an ?', 500, 'litres'),
  estimation('Insolite', 2, 'Combien de dents a un escargot, en milliers ?', 14, 'milliers'),
  estimation('Insolite', 3, 'Combien de pas fait un humain dans toute sa vie, en millions ?',
    150, 'millions'),
  intrus('Insolite', 2, 'Laquelle de ces phobies n’existe pas ?',
    ['Arachnophobie', 'Claustrophobie', 'Anatidaéphobie', 'Agoraphobie', 'Acrophobie'],
    'Anatidaéphobie', 'Elle concernerait les canards'),
  intrus('Insolite', 2, 'Lequel de ces animaux n’existe pas ?',
    ['Ornithorynque', 'Axolotl', 'Blobfish', 'Tarsier', 'Cornebiche'], 'Cornebiche'),
  intrus('Insolite', 2, 'Lequel n’est pas un vrai village de France ?',
    ['Trécon', 'Bèze', 'Cocumont', 'Poil', 'Fripouille'], 'Fripouille'),

  // ═══ Nouvelles questions, tous thèmes ═════════════════════════════════
  ecrite('Géographie', 2, 'Quel pays compte le plus de fuseaux horaires ?',
    ['France', 'La France'], 'Grâce à ses territoires d’outre-mer'),
  ecrite('Géographie', 2, 'Quelle mer n’a aucune côte, entourée de courants ?',
    ['Mer des Sargasses', 'Sargasses']),
  ecrite('Géographie', 2, 'Quel pays a la forme d’une botte ?', ['Italie', 'L’Italie']),
  ecrite('Histoire', 2, 'Quel empereur romain aurait nommé son cheval sénateur ?',
    ['Caligula']),
  ecrite('Histoire', 2, 'Combien de temps a duré la guerre de Cent Ans ?',
    ['116 ans', '116', 'Cent seize ans']),
  ecrite('Sciences', 2, 'Quel organe humain se régénère entièrement ?',
    ['Foie', 'Le foie']),
  ecrite('Sciences', 2, 'Quelle planète tourne sur elle-même en sens inverse des autres ?',
    ['Vénus']),
  ecrite('Sciences', 2, 'Combien de temps met la lumière du Soleil pour nous atteindre ?',
    ['8 minutes', 'Huit minutes', '8 min']),
  ecrite('Cinéma', 2, 'Quel film a rendu célèbre la réplique « Je suis ton père » ?',
    ['Star Wars', 'L’Empire contre-attaque', 'Star Wars V']),
  ecrite('Cinéma', 2, 'Quel animal accompagne Wallace dans les courts-métrages en pâte à modeler ?',
    ['Chien', 'Gromit', 'Un chien']),
  ecrite('Musique', 2, 'Quel instrument compte quatre-vingt-huit touches ?',
    ['Piano', 'Le piano']),
  ecrite('Sport', 2, 'Combien de trous compte un parcours de golf complet ?',
    ['18', 'Dix-huit']),
  ecrite('Sport', 2, 'Dans quel sport peut-on marquer un « strike » ?',
    ['Bowling', 'Le bowling']),
  ecrite('Gastronomie', 2, 'Quel est l’ingrédient principal du guacamole ?',
    ['Avocat', 'L’avocat']),
  ecrite('Technologie', 2, 'Que signifie le « www » d’une adresse web ?',
    ['World Wide Web']),
  ecrite('Langue française', 2, 'Quel mot de la langue française contient les cinq voyelles dans l’ordre ?',
    ['Oiseau'], 'Il vole'),
  ecrite('Animaux', 2, 'Quel animal a les empreintes digitales presque identiques aux nôtres ?',
    ['Koala', 'Le koala']),
  ecrite('Jeux vidéo', 2, 'Quel personnage court en ramassant des anneaux dorés ?',
    ['Sonic']),

  liste('Insolite', 2, 'Citez 4 choses qu’un ours polaire ne mange jamais', 4, [
    'Pingouin', 'Manchot', 'Bambou', 'Cactus', 'Ananas', 'Banane', 'Pizza',
    'Fromage', 'Chocolat', 'Salade', 'Pain', 'Riz', 'Pâtes',
  ], 'Les pingouins vivent à l’autre pôle'),
  liste('Sciences', 2, 'Citez 4 planètes du système solaire', 4, [
    'Mercure', 'Vénus', 'Terre', 'Mars', 'Jupiter', 'Saturne', 'Uranus', 'Neptune',
  ]),
  liste('Gastronomie', 2, 'Citez 5 ingrédients d’une pizza margherita ou napolitaine', 5, [
    'Tomate', 'Mozzarella', 'Basilic', 'Huile d’olive', 'Farine', 'Levure',
    'Sel', 'Eau', 'Origan',
  ]),
  liste('Cinéma', 2, 'Citez 4 films de la saga Star Wars', 4, [
  ...
  liste('Sport', 2, 'Citez 4 sports qui se jouent avec une raquette', 4, [
    'Tennis', 'Badminton', 'Squash', 'Tennis de table', 'Ping-pong',
    'Padel', 'Racquetball', 'Beach tennis', 'Pelote basque',
  ]),
  liste('Musique', 2, 'Citez 4 instruments à vent', 4, [
    'Flûte', 'Trompette', 'Saxophone', 'Clarinette', 'Hautbois', 'Basson',
    'Trombone', 'Tuba', 'Cor', 'Harmonica', 'Cornemuse', 'Flûte de pan',
  ]),

  estimation('Sciences', 2, 'Combien d’os compte le corps humain adulte ?', 206, 'os'),
  estimation('Nature', 2, 'Quelle hauteur atteint le plus haut arbre du monde ?', 116, 'mètres'),
  estimation('Technologie', 2, 'Combien d’heures de vidéo sont mises en ligne sur YouTube chaque minute ?',
    500, 'heures'),
  estimation('Sport', 2, 'Combien de kilomètres compte un marathon ?', 42, 'kilomètres'),

  intrus('Sciences', 2, 'Lequel n’est pas un état de la matière ?',
    ['Solide', 'Liquide', 'Gazeux', 'Plasma', 'Cristallin'], 'Cristallin'),
  intrus('Cinéma', 2, 'Lequel n’est pas un film de Pixar ?',
    ['Là-haut', 'Vice-versa', 'Shrek', 'Ratatouille', 'Coco'], 'Shrek'),
  intrus('Gastronomie', 2, 'Lequel n’est pas un plat italien ?',
    ['Risotto', 'Osso buco', 'Paella', 'Tiramisu', 'Focaccia'], 'Paella'),
  intrus('Musique', 2, 'Lequel n’est pas un instrument à percussion ?',
    ['Batterie', 'Xylophone', 'Timbales', 'Violoncelle', 'Tambourin'], 'Violoncelle'),

  association('Insolite', 2, 'Reliez chaque animal à son cri savant', {
    'Chameau': 'Blatèrement', 'Cerf': 'Brame', 'Corbeau': 'Croassement',
    'Grenouille': 'Coassement',
  }),
  association('Gastronomie', 2, 'Reliez chaque plat à son pays', {
    'Sushi': 'Japon', 'Paella': 'Espagne', 'Goulasch': 'Hongrie',
    'Moussaka': 'Grèce',
  }),
  association('Sciences', 2, 'Reliez chaque planète à sa particularité', {
    'Mars': 'La planète rouge', 'Saturne': 'Ses anneaux',
    'Jupiter': 'La plus grosse', 'Mercure': 'La plus proche du Soleil',
  }),
  association('Sport', 2, 'Reliez chaque sport à son nombre de joueurs par équipe', {
    'Football': '11', 'Basket-ball': '5', 'Volley-ball': '6', 'Rugby à XV': '15',
  }),

  classement('Insolite', 2, 'Classez ces animaux du plus dormeur au plus éveillé',
    ['Koala', 'Chat', 'Humain', 'Éléphant', 'Girafe'],
    'Le plus dormeur', 'Le moins dormeur'),
  classement('Gastronomie', 2, 'Classez ces plats du plus au moins calorique, à poids égal',
    ['Huile d’olive', 'Chocolat', 'Fromage', 'Pain', 'Salade'],
    'Le plus calorique', 'Le moins calorique'),
  classement('Technologie', 2, 'Classez ces réseaux sociaux du plus ancien au plus récent',
    ['Facebook', 'Twitter', 'Instagram', 'Snapchat', 'TikTok'],
    'Le plus ancien', 'Le plus récent'),

  frise('Insolite', 2, 'L’invention du papier toilette moderne', 1857, {
    from: 1700, to: 2000, gap: 45, exact: 5,
    marks: [['Révolution française', 1789], ['Première ampoule électrique', 1879]],
  }),
  frise('Gastronomie', 2, 'La pomme de terre imposée à la table française par Parmentier', 1785, {
    from: 1500, to: 1900, gap: 60, exact: 8,
    marks: [['Découverte de l’Amérique', 1492], ['Révolution française', 1789]],
  }),
  frise('Sport', 2, 'La première ascension de l’Everest', 1953, {
    from: 1900, to: 2000, gap: 12, exact: 1,
    marks: [['Seconde Guerre mondiale', 1939], ['Premier pas sur la Lune', 1969]],
  }),

  themeAuChoix('Insolite', [
    ['Combien de pattes a un mille-pattes, au minimum ?', ['30', 'Trente']],
    ['Quel est le seul continent sans reptile ni serpent ?', ['Antarctique', 'L’Antarctique']],
    ['Quel est l’animal le plus meurtrier pour l’homme ?',
      ['Moustique', 'Le moustique']],
  ]),
  themeAuChoix('Gastronomie', [
    ['De quel pays vient la pizza ?', ['Italie', 'L’Italie']],
    ['Quelle épice est la plus chère au monde ?', ['Safran', 'Le safran']],
    ['Quel fromage italien accompagne traditionnellement les pâtes ?',
      ['Parmesan', 'Parmigiano', 'Le parmesan']],
  ]),
]
