 ...
 *
 * La composition enveloppe × porteuse produit des salves séparées par des
 * silences, ce qui ressemble à de la parole plutôt qu'à du bruit.
 */
const REFERENCE = Array.from({ length: BAR_COUNT }, (_, i) => {
  const envelope = Math.max(0, Math.sin(i * 0.085 + 0.6)) ** 1.4
 ...
 *
 * La piste du haut est la réplique d'origine : figée, elle ne dépend de rien.
 ...
 *
 * Aucune valeur ne transite par l'état React : la boucle d'animation écrit
 * directement dans le DOM, sinon soixante-douze barres déclencheraient un
 * rendu par image.
 */
export function VoiceField() {
  ...
  useEffect(() => {
    ...
    function handlePointer(event: PointerEvent) {
      ...
      intensity = Math.max(0, 1 - Math.abs(event.clientY - centerY) / 420)
    }
  ...
  return (
    <div ref={containerRef} className="w-full select-none" aria-hidden="true">
      <div className="mb-2.5 flex items-end justify-between">
        <span className="eyebrow text-faint">Original</span>
        <span className="eyebrow text-faint tnum">00:11.80</span>
      </div>

      <div className="flex h-20 items-end gap-[3px] sm:h-28">
        {REFERENCE.map((amplitude, i) => (
          <div
            ...
            style={{
              ...
              animation: `wave-rise 620ms ${i * 7}ms both cubic-bezier(.2,.8,.2,1)`,
            }}
          />
        ))}
      </div>
      ...
      <div className="bg-strong my-3.5 h-px w-full sm:my-5" />

      <div className="flex h-20 items-start gap-[3px] sm:h-28">
